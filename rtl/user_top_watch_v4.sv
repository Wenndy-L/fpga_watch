...
// Watch V4.
// V3 plus accurate seconds tick realignment when leaving seconds edit mode.
module user_top_watch_v4 #(
  ...
  // Restart the seconds tick when moving from seconds edit to minutes edit.
  // This makes the next seconds tick occur one full second later.
  assign realign_seconds_tick = mode_enable[0] && button[3];

  restartable_rate_generator #(
      .CYCLE_COUNT(CYCLES_PER_SECOND)
  ) u_divider_1hz (
      .clk (clk),
      .run (!realign_seconds_tick),
      .tick(seconds_tick)
  );
